...
@---------------------BOARD--AND--SCORE--INITIALIZATION-------------------------------@
  mov r0,#1
  mov r1,#1
  ldr R2,=dash @ target is Stdout
  swi 0x204
  mov r0,#1
  mov r1,#2
  ldr R2,=lines @ target is Stdout
  swi 0x204
  mov r0,#1
  mov r1,#3
  ldr R2,=lines @ target is Stdout
  ...
  @ Initializing 1st player pieces
  mov r2,#1
  mov r0,#27
  str r2, [r3,r0,LSL#2]
  mov r0,#36
  ...
  @ Initializing 2nd player pieces
  mov r2,#2
  mov r0,#28
  str r2, [r3,r0,LSL#2]
  mov r0,#35
  ...
  mov r0,#2
  mov r1,#11
  ldr R2,=winner1 @ target is Stdout
  swi 0x204
  b gameended
  scless:
  mov r0,#2
  mov r1,#11
  ldr R2,=winner2 @ target is Stdout
  swi 0x204
  b gameended
  dddd:
  mov r0,#2
...
print_board: @
  ...
  mov r0,#2
  mov r1,#12
  ldr R2,=score1 @ target is Stdout
  swi 0x204
  mov r0,#2
  mov r1,#13
  ldr R2,=score2 @ target is Stdout
  swi 0x204
  mov r0,#27
  mov r1,#12
  ldr r5, =SCORE
...
check_valid: @ i: r0, j: r1 -> ans:r0
  ...
  @if onBoard(i,j)==0 then return 0
  bl onBoard
  mov r1,r0
  mov r0, #0
  cmp r1, #0
...
@-------------------------------------------------------------------------@
onBoard: @x:r0,y:r1 -> r0:(x < 0 || y < 0 || x > 7 || y > 7)
  cmp r0,#0
  blt return_onBoard
  cmp r1,#0
  blt return_onBoard
...
@-------------------------------------------------------------------------@
convert: @ x: r0, y: r1, inc_x: r2, inc_y: r3, set: r4 -> ans: r0
  str r14, [r13,#-4]!
	str r9, [r13,#-4]!
	str r8, [r13,#-4]!
	str r7, [r13,#-4]!
  ...
  @if onBoard(x,y)==0 then return 0
  bl onBoard
  mov r1,r0
  mov r0,#0
	cmp r1, #0
...
@-------------------------------------------------------------------------@
conversions: @ i:r0, j:r1, set:r2 -> ans: r0
  str r14, [r13,#-4]!
  str r8, [r13,#-4]!
  str r7, [r13,#-4]!
  str r6, [r13,#-4]!
...
play: @ x: r0, y: r1 -> ans: r0
  ...
  @if conversions(x,y,1) == 0 then return 0
  mov r0,r3
  mov r1,r4
  mov r2,#1
  bl conversions
...
dash: .asciz "--------------------------"
lines: .asciz "|                        |"
score1: .asciz "Score of Player 1 is "
score2: .asciz "Score of Player 2 is "
winner1: .asciz "Player 1 Won The Game!  Congrats!"
winner2: .asciz "Player 2 Won The Game!  Congrats!"
